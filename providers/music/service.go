...
func (s *Service) GetConcerts() ([]seatgeek.Concert, error) {
	...
	if err != nil {
		log.Fatalf("Error(s) occured during retrieving events: ", err)
	}

	c, err:= s.SeatGeek.GetArtistsConcerts(arts)
	if err != nil {
		if c == nil || len(c) == 0 {
			return nil, fmt.Errorf("Could not retrieve any events: ", err)
		}
		log.Printf("Error(s) occured during retrieving events: ", err)
	}
